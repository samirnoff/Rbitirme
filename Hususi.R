...
grup <- factor(c(rep(c("Yatakda tedavi sayýsý", "Ayakta          \"       \" "), each=length(yatak))),
               levels=c("Yatakda tedavi sayýsý", "Ayakta          \"       \" "))
data <- data.frame("x"=zaman, "y"=deger, "grp"=grup) 
text <- element_text(color = "black", size = 14)
xcor <- rep(paste(c(seq(1926,1937)),sep=""),2)
ycor <- deger + 100
...
ggplot(data=data,aes(x=factor(zaman), y=deger, fill=grup, color=grup, alpha=grup)) +
  labs(title = "HUSUSÝ ÝDARELERE AÝT HASTANELER MESAÝSÝ
  
       ",
       subtitle = "1926 \u2013 1937\n")+
  geom_bar(stat="identity",position =position_dodge(width = 0.2),color="black", size=1.25, width=1.5) +
  theme_classic()+
  ...
  scale_x_discrete(breaks=c(1926,1927,1928,1929,1930,1931,1932,1933,1934,1935,1936,1937))+ 
